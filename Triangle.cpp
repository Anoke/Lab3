...
istream& operator>>(istream& stream, Triangle::Point& a)
{
    stream >> a.x >> a.y;
    return stream;
}
ostream& operator<<(ostream& stream, const Triangle::Point& a)
{
    stream << a.x << " " << a.y;
    return stream;
}
istream& operator>>(istream& stream, Triangle& a)
{
    stream >> a.A.x >> a.A.y >> a.B.x >> a.B.y >> a.C.x >> a.C.y;
    return stream;
}
ostream& operator<<(ostream& stream, const Triangle& a)
{
    stream << "(" << a.A.x << ";" << a.A.y << ")\n(" << a.B.x << ";" << a.B.y << ")\n(" << a.C.x << ";" << a.C.y << ")";
    return stream;
}
